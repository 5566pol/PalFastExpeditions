...
def adjust_time(st: SYSTEMTIME, offset_seconds: float = 0, offset_ms: int = 0) -> SYSTEMTIME:
    """调整时间，返回新的 SYSTEMTIME"""
    total_ms = system_time_to_seconds(st) * 1000 + st.wMilliseconds + int(offset_seconds * 1000) + offset_ms
    ...
    new_st = SYSTEMTIME()
    new_st.wYear = st.wYear
    new_st.wMonth = st.wMonth
    new_st.wDayOfWeek = st.wDayOfWeek
    new_st.wDay = st.wDay
    new_st.wHour = h
    new_st.wMinute = m
    new_st.wSecond = s
    new_st.wMilliseconds = ms
    return new_st


def format_time(st: SYSTEMTIME) -> str:
    return f"{st.wHour:02d}:{st.wMinute:02d}:{st.wSecond:02d}"
